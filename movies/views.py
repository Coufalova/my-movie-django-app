...
def create(request):
    if request.method == 'POST':
        data = {
            ...
            'Pictures': [{'url': request.POST.get('url') or 'http://i.imgur.com/H4GBcEF.png'}],
        ...
        try:
            ...
            messages.waring(request, 'Got an error when trying to create a new movie: {}'.format(e))
    return redirect('/')
...
def edit(request, movie_id):
    if request.method == 'POST':
        ...
        try:
            ...
            messages.warning(request, 'Got an error when trying ot update a movie: {}'.format(e))
    return redirect('/')
